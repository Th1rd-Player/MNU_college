...
class SQLighter:
    ...
    def set_in_rozkla(self,day, groupp, text):
        with self.connection:

            self.cursor.execute("INSERT INTO `rozkla` (`day`, 'groupp','text') VALUES(?,?,?)", (day,groupp,text))

    def add_his(self, user_id,day, text):
        with self.connection:
            self.cursor.execute("INSERT INTO 'histor'('user_id','dayt', 'text') VALUES(?,?,?)",(user_id,day, text))
    def close(self):
